...
export default {
  ...
  actions: {
    async getLoginData({ commit, dispatch }, payload) {
      const APIkey = "AIzaSyD17ImzQcIAvk8tsBjZ7cEKVkFVT97vei4";
      const authUrl =
    ...
    async addNewUser({ commit, state }, payload) {
      try {
        const { data } = await axios.post(
          `https://vue-js-project-665be-default-rtdb.asia-southeast1.firebasedatabase.app/user.json?auth=${state.token}`,
          payload
        );
